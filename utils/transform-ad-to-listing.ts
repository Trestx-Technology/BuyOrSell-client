...
export const transformAdToListingCard = (ad: AD, locale?: Locale): ListingCardProps => {
  ...
  const normalizeExtraFields = (): Record<string, string | number | boolean | string[] | null> => {
    ...
    if (!Array.isArray(ad.extraFields)) {
      return ad.extraFields as Record<string, string | number | boolean | string[] | null>;
    }
  ...
  const getSellerInfo = () => {
    ...
    if (ad.organization?.tradeName) {
      ...
      sellerName = `${ad.owner.firstName || ""} ${ad.owner.lastName || ""}`.trim();
    }

    // Determine if verified - check organization verified status or owner verification
    const isVerified = ad.organization?.verified || 
...
export const transformAdToHotDealsCard = (
  ...
  if (isDealAd) {
    ...
    if (adObj.extraFields) {
      ...
      dealValidThrough = extraFields.dealValidThrough || 
  ...
  if (!isDealAd && discountPercentage && baseCard.price) {
    finalOriginalPrice = Math.round(baseCard.price / (1 - discountPercentage / 100));
  }
  ...
  return {
    ...
    discount: discountPercentage,
    dealValidThrough,
    discountText: discountPercentage ? `FLAT ${Math.round(discountPercentage)}% OFF` : undefined,
...
const transformDealAdToCard = (ad: DealAd, locale?: Locale): ListingCardProps => {
  ...
  const displayPrice = ad.discountedPrice !== null && ad.discountedPrice !== undefined 
    ? ad.discountedPrice 
    : ad.price;
  const originalPrice = ad.discountedPrice !== null && ad.discountedPrice !== undefined 
    ? ad.price 
    : undefined;
