...
export default function CandidateSearchBar() {
  ...
  // Debounce inputs
  const [inputLocation, setInputLocation] = useDebouncedValue(
    location,
    setLocation,
    500,
  );
  const [inputSearch, setInputSearch] = useDebouncedValue(
    searchQuery,
    setSearchQuery,
    500,
  );
  ...
  const { data: jobSuggestions, isPending: jobSuggestionsPending } = useAds(
    searchType === "job" && searchQuery
      ? { search: searchQuery, limit: 10, adType: "JOB" }
      : undefined,
  );

  const { data: candidateSuggestions, isPending: candidateSuggestionsPending } =
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    if (searchType === "job") {
      const searchSlug = inputSearch.trim() ? slugify(inputSearch) : "jobs";
      if (inputSearch.trim()) params.set("search", inputSearch);
      router.push(`/jobs/listing/${searchSlug}?${params.toString()}`);
    } else {
      // Candidate/Applicant search
      params.set("type", "candidate");
      if (inputSearch.trim()) params.set("query", inputSearch);
      if (searchType && searchType !== "applicant")
        params.set("role", searchType);
  ...
  return (
    <form onSubmit={handleSearch} className="w-full max-w-[1080px]">
      <div className="flex flex-col lg:flex-row items-center gap-3 lg:gap-0 lg:bg-white lg:dark:bg-zinc-900 lg:rounded-[14.22px] lg:overflow-hidden lg:h-[72px]">
        {/* Search Type Dropdown */}
        <div className="flex items-center justify-start h-[60px] lg:h-full px-4 lg:border-r border-[rgba(199,199,199,0.6)] dark:border-zinc-800 min-w-[150px] w-full lg:w-auto flex-none lg:flex-1 bg-white dark:bg-zinc-900 rounded-[14.22px] lg:rounded-none lg:bg-transparent lg:shadow-none">
          <Select value={searchType} onValueChange={setSearchType}>
            <SelectTrigger className="border-0 rounded-none h-full px-0 text-[14.22px] focus:ring-0 bg-transparent hover:bg-transparent text-[#8A8A8A] dark:text-zinc-400 w-full shadow-none outline-none">
        ...
        <div className="flex-1 w-full lg:w-auto lg:max-w-[350px] flex items-center h-[60px] lg:h-full px-4 lg:border-r border-[rgba(199,199,199,0.6)] dark:border-zinc-800 relative bg-white dark:bg-zinc-900 rounded-[14.22px] lg:rounded-none lg:bg-transparent lg:shadow-none">
          <Popover
            open={openSearch && !!inputSearch}
            ...
            <PopoverAnchor asChild>
              <div className="relative w-full">
                <input
                  ...
                  placeholder={
                    searchType === "job"
                      ? t.jobs.search.jobPlaceholder
                      : t.jobs.search.candidatePlaceholder
                  }
                  value={inputSearch}
                  onChange={(e) => {
                ...
                />
                {(candidateSuggestionsPending || jobSuggestionsPending) && (
                  <Loader2 className="absolute right-3 text-purple top-1/2 -translate-y-1/2 animate-spin" />
                )}
              </div>
            </PopoverAnchor>
            <PopoverContent
              className="p-1 w-[200px] sm:w-[300px] bg-white dark:bg-zinc-900 border dark:border-zinc-800 shadow-none"
              align="start"
                ...
                {isLoading ? (
                  ...
                  suggestions?.map((item: any, idx: number) => (
                    <button
                      key={idx}
                      type="button"
                      className="text-left px-3 py-2 hover:bg-gray-100 dark:hover:bg-zinc-800 rounded-sm text-sm dark:text-zinc-200 w-full"
                      onClick={() => {
        ...
        <div className="flex-1 w-full lg:w-auto flex items-center pl-4 relative bg-white dark:bg-zinc-900 rounded-[14.22px] lg:rounded-none lg:bg-transparent shadow-sm lg:shadow-none h-[60px] lg:h-full">
          ...
          <Popover
            open={openLocation && (emirates?.length || 0) > 0}
