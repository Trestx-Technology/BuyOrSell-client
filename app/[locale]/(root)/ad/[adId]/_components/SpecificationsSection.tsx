  ...
  const renderSpecItem = (key: string, value: string, index: number) => {
    ...
    if (needsPopover) {
      return (
        <Popover key={index}>
          <PopoverTrigger asChild>
            <div className="flex justify-between items-start group border-gray-100 dark:border-slate-800 w-full p-2 rounded cursor-pointer hover:bg-purple/5 transition-colors">
              <span className="text-sm text-grey-blue dark:text-gray-400 whitespace-nowrap mr-4">
                {key}
              </span>
              {valueContent}
            </div>
