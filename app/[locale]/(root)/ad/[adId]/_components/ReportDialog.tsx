...
const ReportDialog: React.FC<ReportDialogProps> = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success(
        t.ad.report.successMessage || "Report submitted successfully",
      );
      handleClose();
    } catch (error) {
      console.log(t.ad.report.errorMessage || "Failed to submit report");
    }
  };
  ...
  const getTitle = () => {
    switch (reportedType) {
      ...
      default:
        return t.ad.report.title || "Report this ad";
    }
  };
  ...
  const getReasonLabel = () => {
    switch (reportedType) {
      ...
      case "company":
        return "Why are you reporting this company?";
      case "organization":
        return "Why are you reporting this organization?";
      default:
        return t.ad.report.reasonLabel || "Why are you reporting this ad?";
    }
  };

  return (
    <ResponsiveModal open={open} onOpenChange={handleClose}>
      <ResponsiveModalContent className="max-w-md w-[95%] sm:h-[95%] max-h-[650px] sm:max-w-lg border-none bg-white dark:bg-gray-950 p-0 overflow-hidden shadow-2xl">
        <div className="relative overflow-y-auto">
                ...
                {reportReasons.map((item) => (
                  <div
                    ...
                    className={cn(
                      ...
                      reason === item.id &&
                        "border-purple bg-purple/[0.02] dark:bg-purple/[0.05]",
                    )}
                  ...
                  >
                    <RadioGroupItem
                      value={item.id}
                      id={item.id}
                      className="text-purple border-gray-300 dark:border-gray-700"
                    />
                    <Label
                      htmlFor={item.id}
          ...
          <div className="p-6 sticky bottom-0 bg-white dark:bg-gray-950 border-t border-gray-100 dark:border-gray-800  flex gap-3 sm:gap-4 items-center">
            <Button
              variant="ghost"
              onClick={handleClose}
              disabled={isPending}
