...
export const PlanCard: React.FC<PlanCardProps> = ({ plan, perMonthText }) => {
  ...
  // Checkout mutations
  const { mutate: createSubscriptionCheckout, isPending: isSubPending } =
    useCreatePlanSubscriptionCheckout();
  ...
  const handleSubscribe = () => {
    ...
    const isLifetime =
      ...
      plan.validationPeriod.toLowerCase() === "lifetime";
        ...
        },
        {
          onSuccess: (data) => {
            if (data.data.checkoutUrl) {
              router.push(
                `/${locale}/pay?checkoutUrl=${encodeURIComponent(data.data.checkoutUrl)}`,
              );
            } else {
