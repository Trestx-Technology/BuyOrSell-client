...
import {
  ImageGallery,
  ImageItem,
} from "../../details/_components/image-upload";
import {
  VideoUpload,
  type VideoItem,
} from "../../details/_components/video-upload";
...
} from "../../details/_components/MultipleImageInput";
...
import {
  ...
  FormValues,
} from "../../details/_components/DynamicFieldRenderer";
...
export default function EditAdPage() {
  ...
  const initialAdLocation = useMemo(() => {
    ...
    const rawAddr = existingAd.address || existingAd.location;
  ...
  useEffect(() => {
    if (existingAd && category) {
      const defaultValues: any = {
        ...
        // Map images
        images:
          existingAd.images?.map((url, index) => ({
            id: `img-${index}`,
            url: url,
        ...
        exchangeImages: existingAd.exchangeWith?.imageUrl
          ? [
              {
                id: "ex-img-1",
                url: existingAd.exchangeWith.imageUrl,
                file: null,
              },
            ]
          : [],
        ...
        // Map Address
        address: (() => {
          const addr = existingAd.address || existingAd.location;
      ...
      // Handle Address specifically for UI state (MapComponent)
  ...
  // Ownership Check
  useEffect(() => {
    if (!isAdLoading && existingAd && session?.user?._id) {
      const ownerId =
        typeof existingAd.owner === "string"
          ? existingAd.owner
          : existingAd.owner?._id;
      if (ownerId && ownerId !== session.user._id) {
        toast.error("You are not authorized to edit this ad");
        router.push(localePath("/"));
      }
    }
  }, [existingAd, session, isAdLoading, router, localePath]);
  ...
  // Watch form values for dependent fields and preview
  const formValues = watch();

  // Transform form values for live preview
  const previewCardData = useMemo(() => {
    if (!existingAd) return null;

    // Create a mock AD object for transformation
    const mockAd: AD = {
      ...existingAd,
      title: (formValues.title as string) || existingAd.title,
      description: (formValues.description as string) || existingAd.description,
      price: (formValues.price as number) || existingAd.price,
      discountedPrice: formValues.deal
        ? (formValues.discountedPrice as number)
        : undefined,
      images:
        ((formValues.images as ImageItem[])
          ?.map((img) => img.url)
          .filter(Boolean) as string[]) || existingAd.images,
      deal: !!formValues.deal,
      isExchangable: !!formValues.isExchange,
      contactPhoneNumber:
        (formValues.phoneNumber as string) || existingAd.contactPhoneNumber,
    };

    return transformAdToListingCard(mockAd, locale as any);
  }, [formValues, existingAd, locale]);
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    const adType: "AD" | "JOB" = isJobCategory(category?.name || "")
      ? "JOB"
      : "AD";
        ...
        <div className="flex h-full gap-10 relative">
          ...
          <div className="w-full space-y-6 md:w-2/3 h-full pb-24">
            ...
            <div className="space-y-4">
              ...
              {/* Image Upload */}
              <FormField label="Ad Images" htmlFor="images" fullWidth={true}>
                <Controller
                  name="images"
                  control={control}
                  render={({ field }) => (
                      ...
                      {(selectedLocation || initialAdLocation) && (
                        <div className="mt-2 p-3 bg-blue-50 rounded-lg">
                          <p className="text-sm text-blue-800">
                            ...
                            {(selectedLocation || initialAdLocation)?.address}
                          </p>
